...
export const BNB_CHAIN_LOGO_PATH =
...
export const AppLogo: React.FC<AppLogoProps> = ({ 
  className = "w-8 h-8", 
  rounded = "rounded-xl",
  alt = "BinanceHarvest Logo"
}) => {
  return (
    <div 
      className={`relative overflow-hidden shrink-0 flex items-center justify-center bg-[#F0B90B] ${rounded} ${className} shadow-lg shadow-amber-500/20`}
      title={alt}
    >
      <svg 
        viewBox="0 0 24 24" 
        className="w-[68%] h-[68%] fill-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.1)]" 
        xmlns="http://www.w3.org/2000/svg"
      >
        <path d={BNB_CHAIN_LOGO_PATH} fill="#FFFFFF" />
      </svg>
    </div>
  );
};
